Contributing to Widecore
=======

We're working hard to make *widecore* the most powerful JavaScript library for working with widecoin. Our goal is to have *widecore* be a library that can be used by anyone interested in widecoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in widecore that any change must adhere.
...
When possible, widecore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Widecore methods. For example, calling `JSON.stringify` on an Widecore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'widecore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the widecore object.

DO:
```javascript
var widecore = require('../');
var PublicKey = widecore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add widecoin-project git@github.com:widecoin-project/widecore.git
git pull --rebase widecoin-project master
```
...
```
Finally go to [github.com/widecoin-project/widecore](https://github.com/widecoin-project/widecore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of widecore.

If you have any questions feel free to post them to
[github.com/widecoin-project/widecore/issues](https://github.com/widecoin-project/widecore/issues).
